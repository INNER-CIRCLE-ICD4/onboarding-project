...
@Component
public class LongTextAnswerHandler implements AnswerHandler {
    @Override
    public boolean supports(QuestionType type) {
        return type == QuestionType.LONG_TEXT;
    }
    ...
    public void validate(SubmitAnswerRequest answer, Question question) {
        ...
        if (answer.getAnswerText() != null && answer.getAnswerText().length() > 5000) {
            throw new IllegalArgumentException("답변이 너무 깁니다(최대 5000자): " + question.getTitle());
        }
    }
