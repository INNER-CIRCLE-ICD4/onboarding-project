...
class SurveyItemValidatorTest : DescribeSpec({
    describe("SurveyItemValidator 테스트") {
        ...
        context("설명(description) 유효성 검증") {
            ...
            it("빈 설명은 예외가 발생해야 한다") {
                ...
                // then
                exception.message shouldBe ErrorCode.INVALID_SURVEY_ITEM_DESCRIPTION.withArgs(emptyDescription).message
            }
            ...
            it("1000자를 초과하는 설명은 예외가 발생해야 한다") {
                ...
                // then
                exception.message shouldBe ErrorCode.INVALID_SURVEY_ITEM_DESCRIPTION.withArgs(longDescription).message
            }
        }
        ...
        context("표시 순서(displayOrder) 유효성 검증") {
            ...
            it("음수 표시 순서는 예외가 발생해야 한다") {
                ...
                // then
                exception.message shouldBe ErrorCode.INVALID_SURVEY_ITEM_ORDER.withArgs(negativeDisplayOrder.toString()).message
            }
        }
    }
})
