...
class SurveyItemAnswerValidatorTest : DescribeSpec({
    describe("SurveyItemAnswerValidator 테스트") {
        context("질문(question) 유효성 검증") {
            ...
            it("빈 질문은 예외가 발생해야 한다") {
                ...
                // then
                exception.message shouldBe ErrorCode.INVALID_SURVEY_ITEM_ANSWER_QUESTION.withArgs(emptyQuestion).message
            }
            ...
            it("500자를 초과하는 질문은 예외가 발생해야 한다") {
                ...
                // then
                exception.message shouldBe ErrorCode.INVALID_SURVEY_ITEM_ANSWER_QUESTION.withArgs(longQuestion).message
            }
        }
        ...
        context("설명(description) 유효성 검증") {
            ...
            it("빈 설명은 예외가 발생해야 한다") {
                ...
                // then
                exception.message shouldBe ErrorCode.INVALID_SURVEY_ITEM_ANSWER_DESCRIPTION.withArgs(emptyDescription).message
            }
            ...
            it("1000자를 초과하는 설명은 예외가 발생해야 한다") {
                ...
                // then
                exception.message shouldBe ErrorCode.INVALID_SURVEY_ITEM_ANSWER_DESCRIPTION.withArgs(longDescription).message
            }
        }
        ...
        context("입력 타입(surveyInputType) 유효성 검증") {
            ...
            it("빈 입력 타입은 예외가 발생해야 한다") {
                ...
                // then
                exception.message shouldBe ErrorCode.INVALID_SURVEY_ITEM_ANSWER_SURVEY_INPUT_TYPE.withArgs(emptyInputType).message
            }
            ...
            it("유효하지 않은 입력 타입은 예외가 발생해야 한다") {
                ...
                // then
                exception.message shouldBe ErrorCode.INVALID_SURVEY_ITEM_ANSWER_SURVEY_INPUT_TYPE.withArgs(invalidInputType).message
            }
        }
        ...
        context("답변(answer) 유효성 검증") {
            ...
            it("50000자를 초과하는 답변은 예외가 발생해야 한다") {
                ...
                // then
                exception.message shouldBe ErrorCode.INVALID_SURVEY_ITEM_ANSWER_QUESTION.withArgs(longAnswer).message
            }
        }
    }
})
