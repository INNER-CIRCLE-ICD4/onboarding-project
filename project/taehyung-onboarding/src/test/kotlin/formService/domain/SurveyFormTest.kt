...
class SurveyFormTest {
    ...
    fun createTest() {
        ...
        val questions = getSurveyFormQuestions(1, listOf(InputType.SHORT_TEXT))
    ...
    fun modifySurveyFormQuestionTest() {
        ...
        val questions = getFixtureOnlyQuestion(listOf(InputType.SHORT_TEXT))
    ...
    fun modifySurveyFormQuestionRemoveTest() {
        // given
        val surveyForm = getFixtureSurveyForm(listOf(InputType.SHORT_TEXT, InputType.LONG_TEXT))
        val questions =
            getFixtureOnlyQuestion(listOf(InputType.SHORT_TEXT, InputType.LONG_TEXT), isRemoved = true)
    ...
    fun modifySurveyFormQuestionInputTypeTest() {
        // given
        val surveyForm = getFixtureSurveyForm(listOf(InputType.SHORT_TEXT, InputType.LONG_TEXT))
        val questions = getFixtureOnlyQuestion(listOf(InputType.LONG_TEXT, InputType.SHORT_TEXT))
        ...
        // then
        assertThat(surveyForm.questions.size).isEqualTo(questions.size)
        assertThat(surveyForm.questions[0].inputType).isEqualTo(InputType.LONG_TEXT)
        assertThat(surveyForm.questions[1].inputType).isEqualTo(InputType.SHORT_TEXT)
    }
    ...
    fun modifySurveyFormQuestionOptionTest() {
        ...
        val surveyForm =
            getFixtureSurveyForm(
                listOf(InputType.SINGLE_CHOICE),
                isOptions = true,
                optionSize = 3,
            )

        val questions = getFixtureOnlyQuestion(listOf(InputType.SINGLE_CHOICE), isRemoved = false, optionSize = 3)
        ...
        (0..<size).toList().map {
            val question =
                Question(
                    ...
                    inputType = if (inputType == null) InputType.entries[it % 4] else inputType[it],
                    required = true,
                    name = "question$it",
                    options =
                        if (inputType != null &&
                            (
                                inputType[it] == InputType.SINGLE_CHOICE ||
                                    inputType[it] == InputType.MULTI_CHOICE
                            )
                        ) {
                            options
                        } else {
